from AoC2021.day_19 import Scanner, build_beacon_map, largest_distance_between_scanners, parse_scanners
...
def main():
    ...
    scanners = parse_scanners('./data/day_19.txt')
    ...
    # beacon_sets = [s.get_absolute_beacon_set() for s in scanners]

    # # for i in range(len(beacon_sets)):
    # #     diff = beacon_sets[i].difference(valid_beacons)
    # #     if len(diff) == 0:
    # #         print(f'Scanner [{i}] is within parameters....')
    # #     else:
    # #         print(f'Scanner [{i}] is outside of parameters...')

    distance = largest_distance_between_scanners(scanners)
